...
def build_adjacency_list(coords, grid_type, radius=0.1732, max_workers=None):
    ...
    logging.info(f"开始构建邻接表，共分割为 {total_chunks} 个数据块")
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            completed += 1
            if completed % max(1, total_chunks//1) == 0:  #每100%进度输出一次
                logging.info(f"邻接表构建进度: {completed}/{total_chunks} ({100*completed / total_chunks:.1f}%)")
...
def is_connect_fs_log(data, grid_type, start_point, end_point, cpu_number=1):
    """检查起点和终点是否连通"""
    logging.info("开始收敛性检查...")
    if data.empty:
    ...
    for node in adjacency_list:
        ...
        if processed % update_step == 0 or processed == total_nodes:
            ...
            logging.info(f"连通性检查进度: {processed}/{total_nodes} ({progress:.1f}%)")

    logging.info("并查集连通性检查完成")
